    ...
    <div className="screen screen--white" style={{ position: 'relative' }}>
      ...
      <div className="scroll" style={{ padding: '6px 20px 20px' }}>
        <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', textAlign: 'center', padding: '18px 0 22px' }}>
          <span className="cat-icon" style={{ background: catInfo.color, color: catInfo.ink, width: 66, height: 66, borderRadius: 16, fontSize: 28 }}>
            {txn.cat === 'Shopping' ? '🛍️' : txn.cat === 'Dining' ? '🍜' : txn.cat === 'Insurance' ? '🛡️' : txn.cat === 'Transportation' ? '🚗' : '📦'}
          </span>
        ...
        <div>
          <DetailRow label="Category" value={
            <span style={{ display: 'inline-flex', alignItems: 'center', gap: 7 }}>
              <span style={{ width: 9, height: 9, borderRadius: '50%', background: catInfo.color }} /> {txn.cat}
            </span>
          } />
      ...
      {showPicker && (
        <Drawer onClose={() => setShowPicker(false)}>
          <CategoryPicker
            ...
            selected={txn.cat}
            onPick={c => { setTxn(prev => ({ ...prev, cat: c })); setShowPicker(false); }}
          />
        </Drawer>
      )}
    </div>
