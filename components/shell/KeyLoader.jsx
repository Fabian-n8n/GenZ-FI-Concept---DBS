...
// Tight viewBox framing just the key glyph so it sits nicely inside the tile.
const VB = "15 7 26 36";

export default function KeyLoader({ size = 80, boxBg = '#34383f' }) {
  const innerH = size * 0.6;
  const innerW = innerH * (26 / 36);
  return (
    <div style={{
      ...
      <div style={{ position: 'relative', width: innerW, height: innerH }}>
        {/* Faint base key */}
        <svg width={innerW} height={innerH} viewBox={VB} fill="none" style={{ position: 'absolute', inset: 0 }} aria-hidden="true">
          <path d={KEY_D} fill="rgba(255,255,255,0.3)" />
        </svg>
        {/* White fill, swept bottom→top on a loop */}
        <svg width={innerW} height={innerH} viewBox={VB} fill="none" style={{ position: 'absolute', inset: 0, animation: 'keyFill 1.6s cubic-bezier(0.65,0,0.35,1) infinite' }} aria-hidden="true">
